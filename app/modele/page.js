...
<Hero image={asset("/images/model-92.jpg")} alt="NORDHUS 92" title={<>68–156 m².<br/>Cztery punkty wyjścia.</>} text="Każdy model ma określony układ, standard bazowy i cenę początkową. Różnią się liczbą modułów, powierzchnią zabudowy i sposobem organizacji strefy prywatnej."/>
...
<section className="section-space bg-white">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-4"><p className="eyebrow">Porównanie</p><h2 className="display mt-5 text-4xl md:text-5xl">Cztery modele w jednym miejscu.</h2></div>
   <div className="col-span-12 overflow-x-auto lg:col-span-8 lg:col-start-5">
    <table className="w-full min-w-[900px] border-collapse text-left"><thead><tr className="border-b border-[var(--ink)]"><th className="py-4">Model</th><th>Metraż</th><th>Sypialnie</th><th>Łazienki</th><th>Wysokość</th><th>Moduły</th><th>Montaż</th><th>Cena od</th></tr></thead><tbody>{models.map(m=><tr key={m.slug} className="border-b border-[var(--line)]"><td className="py-5 font-normal">{m.name}</td><td>{m.area} m²</td><td>{m.bedrooms}</td><td>{m.bathrooms}</td><td>{m.height}</td><td>{m.modules}</td><td>3–5 dni</td><td>{m.price}</td></tr>)}</tbody></table>
   </div>
  </div>
 </Container>
</section>
...
<section className="section-space">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-5"><p className="eyebrow">Jak wybrać</p><h2 className="display mt-5 text-4xl md:text-5xl">Nie zaczynaj od największego metrażu.</h2></div>
   <div className="col-span-12 space-y-5 lg:col-span-6 lg:col-start-7">
