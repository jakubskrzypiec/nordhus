...
<Hero image={asset("/images/o-nas.jpg")} alt="Pracownia projektowa NORDHUS" title={<>Projekt. Produkcja.<br/>Montaż w jednym rytmie.</>} text="NORDHUS to demonstracyjna marka pokazująca, jak może działać firma produkująca prefabrykowane domy modułowe."/>

<section className="section-space">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-4"><p className="eyebrow">Historia — dane demonstracyjne</p><h2 className="display mt-5 text-4xl md:text-5xl">Zaczęliśmy od problemu z harmonogramem.</h2></div>
...
<section className="section-space overflow-hidden">
 <Container>
  <div className="grid grid-cols-12 gap-y-12">
   ...
   <div className="col-span-12 lg:col-span-4 lg:col-start-9"><p className="eyebrow">Zespół — osoby fikcyjne</p><h2 className="display mt-5 text-4xl md:text-5xl">Kto odpowiada za kolejne decyzje.</h2><div className="mt-8">{[["Anna Nowak","architekt prowadząca"],["Marek Zieliński","konstruktor"],["Karolina Wójcik","kierowniczka produkcji"],["Piotr Lewandowski","koordynator montażu"]].map(([n,r])=><div key={n} className="border-t border-[var(--line)] py-5"><h3 className="text-lg font-normal">{n}</h3><p className="mt-1 text-sm text-[var(--muted)]">{r}</p></div>)}</div></div>
  </div>
 </Container>
</section>
