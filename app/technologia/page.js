...
export default function Page(){
 ...
 return <>
<Hero image={asset("/images/produkcja.jpg")} alt="Produkcja modułów w hali" title={<>Dom powstaje w hali.<br/>Warstwa po warstwie.</>} text="Prefabrykacja nie zmienia fizyki budynku. Zmienia miejsce, w którym powstają przegrody, instalacje i większość wykończenia."/>
  ...
  <div className="grid grid-cols-12 gap-y-12">
   <div className="col-span-12 lg:col-span-7">
    <div className="relative aspect-[4/3] w-full overflow-hidden lg:ml-[calc(-1*max(0px,(100vw-1280px)/2))] lg:w-[calc(100%+max(0px,(100vw-1280px)/2))]">
     <Image src={asset("/images/produkcja.jpg")} alt="Prefabrykowane moduły w hali" fill className="object-cover" sizes="(min-width:1024px) 62vw,100vw" placeholder="blur" blurDataURL={BLUR_DATA_URL}/>
    </div>
   </div>
   <div className="col-span-12 lg:col-span-4 lg:col-start-9">
    <p className="eyebrow">Konstrukcja</p>
...
<section className="section-space bg-white">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-4"><p className="eyebrow">Przekrój ściany</p><h2 className="display mt-5 text-4xl md:text-5xl">Od elewacji do wnętrza.</h2></div>
   <div className="col-span-12 lg:col-span-7 lg:col-start-6">{wall.map(([n,a,b])=><div key={n} className="grid gap-3 border-t border-[var(--line)] py-5 md:grid-cols-[70px_1fr_1fr]"><p className="text-sm text-[var(--muted)]">{n}</p><h3 className="font-normal">{a}</h3><p className="text-[var(--muted)]">{b}</p></div>)}</div>
  </div>
 </Container>
</section>
...
<section className="section-space">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-4"><p className="eyebrow">Izolacyjność cieplna</p><h2 className="display mt-5 text-4xl md:text-5xl">Projektujemy poniżej dopuszczalnych wartości U.</h2></div>
   <div className="col-span-12 lg:col-span-7 lg:col-start-6">
    ...
    <div className="mt-10 grid grid-cols-2 gap-px bg-[var(--line)] md:grid-cols-4">{[["Ściana","U ok. 0,15","WT max 0,20"],["Dach","U ok. 0,12","WT max 0,15"],["Podłoga","U ok. 0,18","WT max 0,30"],["Okna","Uw ≤ 0,90","WT max 0,90"]].map(([a,b,c])=><div key={a} className="bg-white p-5"><p className="text-xs uppercase tracking-[.12em] text-[var(--muted)]">{a}</p><p className="mt-3 text-xl font-normal">{b}</p><p className="mt-2 text-xs text-[var(--muted)]">{c}</p></div>)}</div>
   </div>
  </div>
 </Container>
</section>
...
<section className="section-space bg-white">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-5"><p className="eyebrow">Instalacje</p><h2 className="display mt-5 text-4xl md:text-5xl">Ogrzewanie i wentylacja są częścią projektu modułu.</h2></div>
...
<section className="section-space">
 <Container>
  <div className="grid grid-cols-12 gap-y-10">
   <div className="col-span-12 lg:col-span-4"><p className="eyebrow">Prefabrykacja a budowa tradycyjna</p><h2 className="display mt-5 text-4xl md:text-5xl">Różnica jest głównie w organizacji procesu.</h2></div>
