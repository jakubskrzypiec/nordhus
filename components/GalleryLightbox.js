...
export default function GalleryLightbox({items,className=""}){
 ...
 return <>{<div className={className}>{items.map((item,i)=><button key={item.src+i} type="button" onClick={()=>open(i)} className={`gallery-item group relative overflow-hidden text-left ${item.className||""}`} aria-label={`Powiększ: ${item.alt}`}><Image src={item.src} alt={item.alt} fill className="object-cover transition-transform duration-700 ease-out group-hover:scale-[1.04]" sizes={item.sizes||"(min-width:768px) 50vw,100vw"}/><span className="gallery-cursor" aria-hidden="true">Powiększ</span>{item.overlay}</button>)}</div>}
 {index!==null&&<div ref={dialog} role="dialog" aria-modal="true" aria-label="Podgląd zdjęcia" className="fixed inset-0 z-[100] flex items-center justify-center bg-[#111]/95 p-5" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}>
  ...
  <div className="relative h-[82vh] w-[88vw]"><Image src={items[index].src} alt={items[index].alt} fill quality={90} className="object-contain" sizes="90vw"/></div>
